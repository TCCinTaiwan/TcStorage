 ...
 * @since 0.1.15 2019-09-01 TCC: [調整] Autoplay 提示
 * @since 0.1.15 2022-11-20 TCC: [調整]增加空行以利於可讀性
 * @todo ZIP檔案中，讀取內容
 */
let mouseInfo = {
  down: {
  ...
  if (mouseInfo.down.x != null) {
    if (evt.buttons == 0) { // 沒有按鍵
      ...
      function animate(element, propertie, value, _step = 100, stop_condition = null) { // jQuery.animate模擬
        ...
        let step = (value < element[propertie] ? -1 : 1) * Math.abs(_step),
          id = null;
      ...
      if (evt.y < fileList.clientTop + 50) {
        animate(fileList, "scrollTop", 0, 35, function() {
          return mouseInfo.y >= fileList.clientTop + 50
        }); // 往上
      } else if (evt.y > fileList.clientTop + fileList.clientHeight - 50) {
        animate(fileList, "scrollTop", fileList.scrollHeight - fileList.offsetHeight, 35, function() {
          return mouseInfo.y <= fileList.clientTop + fileList.clientHeight - 50
        }); // 往下
      }
